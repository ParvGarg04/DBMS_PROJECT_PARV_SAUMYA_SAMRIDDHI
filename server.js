...
async function sendEmail(to, subject, html) {
  if (!process.env.SMTP_USER) {
    console.log('[EMAIL SKIPPED — no SMTP configured]', subject, '->', to);
    return;
  }
  try {
...
async function queueNotification(conn, sap_id, email, type, subject, html, reference_id = null, send_at = null) {
  try {
    ...
    // Mark as sent
    await conn.execute(
      `UPDATE Notification SET status='sent', sent_at=NOW() WHERE sap_id=? AND type=? AND reference_id=? ORDER BY created_at DESC LIMIT 1`,
      [sap_id, type, reference_id]
    );
  } catch(e) {
